...
/*
该函数将计算规划的起点以及拼接轨迹的信息输入,上一周期的规划结果信息pre_trajectory
x ,y,heading,kappa,velocity,accel,time, 本周期的绝对时间current_time,
定位信息host x,y,heading,kappa,vx,vy,ax,ay, 输出规划起点信息plan_start
x,y,heading,kappa,vx,vy,ax,ay(vx,vy,ax,ay)。是世界坐标系的待拼接的轨迹信息，一般在上一周期的轨迹中的current_time+100ms，往前取20个点
...
void EMPlanner::CalPlaningStartPoint(const Trajectory &pre_traj,
  ...
  if (pre_traj.trajectory_points().empty()) {
    ...
    if ((lon_err > 2.5) || (lat_err > 0.5)) {
      ...
      // cur_time + dt
      /*
      计算拼接轨迹表示 current_time + 0.1 的时间点在 pre_trajectory_time的编号
      拼接是往从j开始，往前拼接20个，也就是pre_trajectory(j-1),j-2,j-3,....j-19
      分两种情况，如果j小于20，意味着前面的轨迹点不够20个， 如果j >=
      20,意味着前面的点多于20个 还有一个细节需要考虑，pre_trajectory x(j)
      y(j)是规划的起点，那么在轨迹拼接时需不需要在stitch_trajectory中把pre_trajectory
      x(j) y(j)
      ....包含进去答案是否定的，不应该包进去，因为规划的起点会在规划模块计算完成后的结果中包含，如果在拼接的时候
      还要包含，那就等于规划起点包含了两次除非本周期计算的轨迹不包含规划起点，那么stitch_trajectory可以包含规划起点。
      如果本周期计算的轨迹包含规划起点，那么stitch_trajectory就不可以包含规划起点。我们选择规划包含起点，拼接不包含起点的做法.
      */
      pre_index_desire--;
